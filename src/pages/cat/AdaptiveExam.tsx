...
> = {
  full: {
    label: "Full Mock",
    description: "Complete adaptive test across all subjects",
  },
  sectional: {
    label: "Sectional",
...
export default function AdaptiveExam() {
  ...
  const startExamMutation = useMutation({
    mutationFn: (config: StartAdaptiveExamPayload) =>
      catApi.startAdaptiveExam(config),
    onSuccess: (data) => {
      setExamState({
        examId: data.exam_id,
        currentQuestion: data.first_question,
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      if (result.is_exam_complete) {
        const examResults = await catApi.completeAdaptiveExam(
          examState.examId!,
        );
        setExamState((s) => ({
          ...s,
      ...
      >
        ...
        >
          <div className="flex items-center gap-4">
            <Badge
              variant="outline"
              className={`${difficultyColors[examState.currentQuestion.difficulty_label].bg} ${difficultyColors[examState.currentQuestion.difficulty_label].text}`}
            >
              {
                difficultyColors[examState.currentQuestion.difficulty_label]
                  .label
              }
            </Badge>
        ...
        <Progress
          value={(examState.progress.answered / examState.progress.total) * 100}
          className="h-2"
        />
        ...
        <AnimatePresence mode="wait">
          {!showFeedback ? (
                ...
                <CardContent className="pt-6">
                  <div className="space-y-6">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      Expected:{" "}
                      {examState.currentQuestion.expected_time_seconds}s
                    </div>
                    ...
                    {examState.currentQuestion.question_type === "mcq" &&
            ...
            examState.lastFeedback && (
              ...
              >
                <Card
                  className={`border-2 ${examState.lastFeedback.is_correct ? "bg-green-500/10 border-green-500/30" : "bg-red-500/10 border-red-500/30"}`}
                >
                  <CardContent className="pt-6 space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Correct Answer:{" "}
                          <strong>
                            {examState.lastFeedback.correct_answer}
                          </strong>
                        </p>
                      </div>
                    </div>
                    ...
                    <p className="text-sm">
                      {examState.lastFeedback.solution_text}
                    </p>

                    <div className="flex items-center gap-4 pt-2">
                      <Badge
                        variant="outline"
                        className={`${difficultyColors[examState.lastFeedback.difficulty_label].bg} ${difficultyColors[examState.lastFeedback.difficulty_label].text}`}
                      >
                        Next:{" "}
                        {
                          difficultyColors[
                            examState.lastFeedback.difficulty_label
                          ].label
                        }
                      </Badge>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Take a quick diagnostic test to calibrate the adaptive
                    system to your skill level. This helps us provide questions
                  ...
                  >
                    {startDiagnosticMutation.isPending
                      ? "Starting..."
                      : "Start Diagnostic"}
                  </Button>
                </div>
        ...
        <>
                ...
                >
                  <CardContent className="pt-6">
                    ...
                    <p className="text-3xl font-bold">
                      {score.ability_score.toFixed(1)}
                    </p>
                    <Progress
                      value={(score.ability_score / 5) * 100}
                      className="h-2 mt-2"
                    />
                  </CardContent>
                </Card>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-semibold">
                        {predictedScore.low_estimate} -{" "}
                        {predictedScore.high_estimate}
                      </p>
          ...
          <motion.div variants={itemVariants}>
            <h2 className="text-xl font-semibold mb-4">
              Start an Adaptive Exam
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(
                Object.entries(examTypeLabels) as [
                  AdaptiveExamType,
                  (typeof examTypeLabels)[AdaptiveExamType],
                ][]
              ).map(([type, info]) => (
                <Card
                  key={type}
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setExamConfig((c) => ({ ...c, question_count: parseInt(v) }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setExamConfig((c) => ({
                    ...c,
                    time_limit_minutes: parseInt(v) || undefined,
                  }))
                }
              >
                <SelectTrigger>
    ...
    >
      ...
      <Card className="bg-card/50 border-border/50">
        ...
        <CardContent>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Confidence:{" "}
            {(results.predicted_cat_score.confidence * 100).toFixed(0)}%
          </p>
        </CardContent>
      </Card>
