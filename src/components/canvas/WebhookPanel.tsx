...
export const WebhookPanel = memo(
  ({ workflowId, onClose }: WebhookPanelProps) => {
    const { toast } = useToast();
    const [webhookUrl, setWebhookUrl] = useState<string | null>(null);
    const [secret, setSecret] = useState<string | null>(null);
    const [hmacAlgo, setHmacAlgo] = useState<"sha256" | "sha1" | "none">(
      "sha256",
    );
    ...
    const fetchOrCreate = useCallback(async () => {
      ...
      try {
        const res = await api.get(
          `/api/canvas-workflows/${workflowId}/webhook`,
        );
        if (res.success && res.data) {
          ...
          setRateLimit(
            res.data.rate_limit || { max: 10, window: 60, current: 0 },
          );
        }
      } catch {
    ...
    const generateWebhook = useCallback(async () => {
      ...
      try {
        const res = await api.post(
          `/api/canvas-workflows/${workflowId}/webhook/generate`,
          {
            hmac_algo: hmacAlgo,
          },
        );
        if (res.success && res.data) {
          ...
          toast({
            ...
            description: "Your secret URL is ready to use.",
          });
        }
      } catch (err: any) {
        toast({
    ...
    const rotateSecret = useCallback(async () => {
      ...
      try {
        const res = await api.post(
          `/api/canvas-workflows/${workflowId}/webhook/rotate`,
        );
        if (res.success && res.data) {
          setSecret(res.data.secret);
          toast({
            title: "Secret rotated",
            description: "New HMAC secret generated. Update your integrations.",
          });
        }
      } catch (err: any) {
        toast({
    ...
    const usagePercent = Math.min(
      (rateLimit.current / rateLimit.max) * 100,
      100,
    );
              ...
              <div className="py-6 flex flex-col items-center gap-3 text-center">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Generate a secret URL to trigger this workflow from external
                    services.
                  </p>
                </div>
              </div>
            ...
            <>
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60">
                  ...
                  <span>
                    ...
                    <code className="font-mono text-violet-400">
                      X-Signature-{hmacAlgo.toUpperCase()}
                    </code>{" "}
                    header
                  </span>
                </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-[10px] font-bold ${usagePercent > 80 ? "text-rose-400" : "text-muted-foreground"}`}
                  >
                    {rateLimit.current} / {rateLimit.max} per {rateLimit.window}
                    s
                  </span>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {hits.slice(0, 8).map((hit, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-muted/15 border border-border/10"
                      >
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${hit.status === "ok" ? "bg-emerald-500" : "bg-rose-500"}`}
                          />
