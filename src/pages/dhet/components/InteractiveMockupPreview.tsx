...
export const InteractiveMockupPreview: React.FC<
  InteractiveMockupPreviewProps
> = ({ proposal, deviceFrame }) => {
  ...
  const rawContext =
    `${proposal?.title || ""} ${proposal?.ai_image_prompt || ""} ${proposal?.plain_text_spec || ""}`.toLowerCase();

  const isSpoon =
    rawContext.includes("spoon") ||
    rawContext.includes("horse") ||
    rawContext.includes("equi");
  const isCoffee =
    rawContext.includes("coffee") ||
    rawContext.includes("espresso") ||
    rawContext.includes("brew");
  const isDashboard =
    rawContext.includes("dashboard") ||
    rawContext.includes("analytics") ||
    rawContext.includes("stats");
  ...
  return (
    ...
    >
      {/* ── STATUS BAR ──────────────────────────────────────────────────────── */}
      <div
        className="flex items-center justify-between px-5 pt-3 pb-1 text-[11px] font-semibold"
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div
            className="w-4 h-2 rounded-[2px] border p-[1.5px] flex items-center"
      ...
      <div className="flex-1 flex flex-col gap-3 p-4">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            >
              {sliderVal}
              {isSpoon ? " cm" : isCoffee ? " clicks" : isDashboard ? "%" : ""}
            </span>
          </div>
      ...
      <div className="h-4 flex items-center justify-center pb-2">
        <div
          ...
          style={{ backgroundColor: `${textSecondary}50` }}
        />
      </div>
    </div>
  );
