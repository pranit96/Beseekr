          ...
          <section className="rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2">
                ...
                <input
                  type="text"
                  placeholder="e.g. PR on deadlift"
                  value={form.notes}
                  onChange={(e) =>
        ...
        <section className="rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
          ...
          {isLoading ? (
            ...
            <p className="text-xs text-muted-foreground">
              No workouts logged yet. Hit "Log workout" to start.
            </p>
          ) : (
            <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
              {sessions.map((s: any) => (
                <div
                  key={s.id}
                  className="rounded-xl bg-background/60 border border-border/60 p-3 flex items-start justify-between gap-2"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold capitalize">
                      {s.type || "workout"} ·{" "}
                      {s.focus?.replace(/_/g, " ") || "general"}
                    </p>
