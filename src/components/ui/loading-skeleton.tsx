  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className={skeletonClass} role="status" aria-label="Loading..." />
      ))}
    </>
  );
};
...
export const MessageSkeleton = () => (
  <div className="flex gap-4 justify-start animate-fade-in" role="status" aria-label="Loading message">
...
export const AgentCardSkeleton = () => (
  <div className="p-3 rounded-lg border border-border space-y-2 animate-pulse" role="status" aria-label="Loading agent">
...
export const ConversationSkeleton = () => (
  <div className="p-3 rounded-lg space-y-2 animate-pulse" role="status" aria-label="Loading conversation">
    ...
    {Array.from({ length: rows }).map((_, rowIndex) => (
      <div key={`row-${rowIndex}`} className="flex gap-4 py-2">
        {Array.from({ length: columns }).map((_, colIndex) => (
          <LoadingSkeleton key={`cell-${rowIndex}-${colIndex}`} className="h-4 flex-1" />
        ))}
      </div>
    ))}
  </div>
...
export const PageSkeleton = () => (
  <div className="space-y-6 p-6 animate-fade-in" role="status" aria-label="Loading page">
