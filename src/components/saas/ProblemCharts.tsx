...
export function OpportunityScoreChart({ score, maxScore = 100, className }: OpportunityScoreChartProps) {
...
export function MarketSizeChart({ tam, sam, som, className }: MarketSizeChartProps) {
    const data = [
        { name: 'TAM', value: tam?.value || 0, display: tam?.display || 'N/A', fill: '#22c55e' },
        { name: 'SAM', value: sam?.value || 0, display: sam?.display || 'N/A', fill: '#3b82f6' },
        { name: 'SOM', value: som?.value || 0, display: som?.display || 'N/A', fill: '#8b5cf6' },
    ].filter(d => d.value > 0);
    ...
    return (
        <Card className={className}>
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Market Size Breakdown</CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
                <div className="h-[100px] sm:h-[120px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={normalizedData} layout="vertical" margin={{ left: 0, right: 0 }}>
...
export function ValidationBreakdownChart({ breakdown, className }: ValidationBreakdownChartProps) {
        ...
        <Card className={className}>
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium flex items-center justify-between">
                    ...
                    <span className="text-primary font-bold">{total}/{maxTotal}</span>
                </CardTitle>
            </CardHeader>
                        ...
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: item.value / item.max >= 0.6 ? '#22c55e' :
                                        item.value / item.max >= 0.3 ? '#f59e0b' : '#ef4444'
                                }}
                            />
                        </div>
                    </div>
...
export function TrendSparkline({ data, className }: TrendSparklineProps) {
    ...
    const isGrowing = data.length >= 2 && data[data.length - 1].frequency > data[0].frequency;
            ...
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium flex items-center justify-between">
                    ...
                    <span className={cn(
                        ...
                        isGrowing ? "bg-green-500/10 text-green-600" : "bg-amber-500/10 text-amber-600"
                    )}>
                        ...
                        <AreaChart data={data} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                            <defs>
                                <linearGradient id="trendGradient" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="5%" stopColor={isGrowing ? '#22c55e' : '#f59e0b'} stopOpacity={0.3} />
                                    <stop offset="95%" stopColor={isGrowing ? '#22c55e' : '#f59e0b'} stopOpacity={0} />
                                </linearGradient>
                            </defs>
                            <Area
                                type="monotone"
...
export function ScoreFactorsChart({ factors, className }: ScoreFactorsChartProps) {
    ...
    const data = factors.map(f => ({
        ...
        value: f.score,
        weight: f.weight,
        fill: f.impact === 'high' ? '#22c55e' : f.impact === 'medium' ? '#3b82f6' : '#94a3b8'
    }));
