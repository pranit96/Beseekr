...
import { GitFork, Plus, Trash2, ChevronDown, ChevronUp, Cpu } from "lucide-react";
...
const SplitNode = memo(({ data, selected }: NodeProps) => {
  ...
  const addBranch = () => {
    const newBranches = [...branches, { label: `Branch ${branches.length + 1}`, instruction: "" }];
  ...
  const updateBranch = (idx: number, field: keyof Branch, value: string) => {
    const updated = branches.map((b, i) => i === idx ? { ...b, [field]: value } : b);
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 pt-3 pb-2.5 border-b border-cyan-500/15">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">Fan-Out</div>
      ...
      <div className="px-3.5 py-3 space-y-2">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-cyan-500/8 border border-cyan-500/15">
          ...
          <span className="text-[11px] font-semibold text-cyan-300/80 truncate">{agentName}</span>
        </div>

        {expanded && (
          <div className="space-y-1 pb-2 border-b border-cyan-500/10">
            <label className="text-[9px] font-black text-cyan-400/50 uppercase tracking-widest">Shared Agent (Optional)</label>
            <select
              value={(data.agentId as string) || ""}
              onChange={e => {
                const selectedId = e.target.value;
                const selectedAgent = agents.find((a: any) => a.id === selectedId);
                (data.onAgentChange as Function)?.(selectedId, selectedAgent ? selectedAgent.name : "None (Shared)");
              }}
            ...
            <div className="flex gap-0.5">
              {branches.map((_, i) => (
                <div
                  ...
                  style={{ animation: `bar-bounce 0.8s ease-in-out infinite`, animationDelay: `${i * 0.15}s` }}
                />
              ))}
            </div>
            <span className="text-[10px] font-bold text-cyan-400">Running {branches.length} branches in parallel…</span>
          </div>
        )}

        {expanded && branches.map((branch, idx) => (
          <div
            key={idx}
          ...
          >
            <div className="flex items-center gap-2">
              <div className={`w-2.5 h-2.5 rounded-full bg-gradient-to-br ${BRANCH_COLORS[idx % BRANCH_COLORS.length]} shrink-0`} />
              <input
                value={branch.label}
            ...
            <textarea
              ...
              onChange={e => updateBranch(idx, "instruction", e.target.value)}
