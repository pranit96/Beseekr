...
export default function WellnessNutrition() {
  ...
  const totals = (foodData as any)?.totals ?? {
    calories: 0,
    protein: 0,
    carbs: 0,
    fat: 0,
  ...
  const handleManual = async () => {
    ...
    try {
      toast({
        title: "Estimating macros…",
        description: "AI is analysing your meal.",
      });
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-4 sm:py-6 space-y-4 sm:space-y-5">
        ...
        <section className="rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setManualForm((f) => ({ ...f, serving: e.target.value }))
                    }
                    className="w-full mt-1 px-2.5 py-2 rounded-lg bg-background border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setManualForm((f) => ({ ...f, mealType: e.target.value }))
                    }
                    className="w-full mt-1 px-2.5 py-2 rounded-lg bg-background border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                  >
                    {MEAL_TYPES.map((t) => (
          ...
          <p className="text-[11px] text-muted-foreground">
            {isMobile ? 'iOS tip: choose "Camera" when prompted. ' : ""}Limit:
            10 image analyses/hour.
          </p>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
          ...
          {logsLoading ? (
            ...
            <p className="text-xs text-muted-foreground">
              No meals logged today. Use camera or type a meal above.
            </p>
          ) : (
