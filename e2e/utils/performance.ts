...
export function checkPerformanceThresholds(metrics: PerformanceMetrics): { passed: boolean; violations: string[] } {
    const violations: string[] = [];

    if (metrics.pageLoadTime > PERFORMANCE_THRESHOLDS.pageLoad) {
        violations.push(`Page load time ${metrics.pageLoadTime}ms exceeds threshold ${PERFORMANCE_THRESHOLDS.pageLoad}ms`);
    }

    if (metrics.timeToInteractive > PERFORMANCE_THRESHOLDS.tti) {
        violations.push(`Time to Interactive ${metrics.timeToInteractive}ms exceeds threshold ${PERFORMANCE_THRESHOLDS.tti}ms`);
    }

    if (metrics.largestContentfulPaint > PERFORMANCE_THRESHOLDS.lcp) {
        violations.push(`LCP ${metrics.largestContentfulPaint}ms exceeds threshold ${PERFORMANCE_THRESHOLDS.lcp}ms`);
    }
...
export function generateReport(url: string, metrics: PerformanceMetrics): PerformanceReport {
