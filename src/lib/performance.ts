...
class PerformanceMonitor {
  ...
  getSummary(): {
    ...
    const slowest = completed.reduce((prev, current) => {
  ...
  reportWebVitals(): void {
    ...
    if (navTiming) {
      logger.info('Navigation Timing', {
        domContentLoaded: (navTiming.domContentLoadedEventEnd - navTiming.domContentLoadedEventStart).toFixed(2) + 'ms',
        loadComplete: (navTiming.loadEventEnd - navTiming.loadEventStart).toFixed(2) + 'ms',
        domInteractive: (navTiming.domInteractive - navTiming.fetchStart).toFixed(2) + 'ms'
      });
    }
  }
}
...
export const perf = {
  ...
  measure: async <T>(name: string, fn: () => T | Promise<T>, metadata?: Record<string, any>) => {
