...
export const AgentSelector = ({
  ...
  const renderContent = () => (
    <div className="space-y-5">
      <div className={cn("flex items-center justify-between gap-3 pb-2", isNewUI && "border-b border-white/[0.04]")}>
        <div className="flex-1">
          <h4 className={cn("font-bold tracking-tight", isNewUI ? "text-lg text-foreground" : "text-sm")} id="agent-selector-title">
            Select Agents
          </h4>
          <p className="text-xs text-muted-foreground/60" id="agent-selector-description">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className={cn(
                ...
                isNewUI ? "rounded-xl border-white/[0.08] bg-black/20 backdrop-blur-md" : "h-9"
              )}
            />
          </div>
        </div>
      ...
      <div className="custom-scrollbar max-h-[60vh] overflow-y-auto pr-1 space-y-6">
        ...
        {filteredCustomAgents.length > 0 && (
          <div role="group" aria-labelledby="custom-agents-heading">
            <div className="flex items-center gap-2 mb-3">
              <h5 id="custom-agents-heading" className="text-[11px] font-black uppercase tracking-widest text-muted-foreground/80">
                Custom Agents
              </h5>
              ...
              {filteredCustomAgents.map((agent) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-2.5 h-2.5 rounded-full ring-4 ring-white/[0.03]" style={{ backgroundColor: getAgentColor(agent.id) }} />
                    <div className="flex-1 flex flex-col gap-0.5 w-full">
                      <span className="text-[13px] font-bold text-foreground/90 truncate w-full">{agent.name}</span>
                      <span className="text-[10px] text-muted-foreground/70 line-clamp-2 leading-snug">{agent.description}</span>
                    </div>
                  </button>
                );
              })}
        ...
        <div role="group" aria-labelledby="default-agents-heading">
          <div className="flex items-center gap-2 mb-3">
            <h5 id="default-agents-heading" className="text-[11px] font-black uppercase tracking-widest text-muted-foreground/80">
              Available Agents
            </h5>
            <div className="h-px flex-1 bg-gradient-to-r from-white/[0.06] to-transparent" />
          </div>
          ...
          {filteredDefaultAgents.length === 0 && searchQuery ? (
            <div className="text-center py-8 bg-white/[0.01] rounded-xl border border-white/[0.04]" role="status">
    ...
    return (
      <Dialog>
        ...
        <DialogContent className="w-full max-w-4xl p-6 bg-[#09090b]/95 border-white/[0.08] shadow-[0_0_50px_-12px_rgba(0,0,0,0.7)] rounded-2xl backdrop-blur-3xl outline-none">
          <DialogHeader className="hidden"><DialogTitle>Select Agents</DialogTitle><DialogDescription>Choose agents.</DialogDescription></DialogHeader>
          {renderContent()}
        </DialogContent>
      </Dialog>
    );
