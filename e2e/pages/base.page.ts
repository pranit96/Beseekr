...
export class BasePage {
    ...
    constructor(page: Page) {
        this.page = page;
        this.loadingSpinner = page.locator('[data-testid="loading"], .animate-spin, [class*="Loader"]');
        this.toastNotification = page.locator('[data-sonner-toast], [role="status"]');
        this.errorMessage = page.locator('[class*="destructive"], [class*="error"]');
    }
    ...
    async expectErrorToast(message?: string) {
        const errorToast = this.page.locator('[data-sonner-toast][data-type="error"], [class*="destructive"]');
        await expect(errorToast.or(this.toastNotification)).toBeVisible();
        if (message) {
            await expect(errorToast.or(this.toastNotification)).toContainText(message);
        }
    }
    ...
    async waitForLoadingToFinish() {
        ...
        await this.loadingSpinner.waitFor({ state: 'detached', timeout: 30000 }).catch(() => { });
    }
