...
import { StudyTask, CreateTaskPayload, TaskType, TaskPriority, TaskStatus } from '@/types/cat';
...
export default function Tasks() {
    ...
    const groupedTasks = useMemo(() => {
        ...
        const today = pending.filter(t => t.deadline && isToday(new Date(t.deadline)));
        const upcoming = pending.filter(t => !t.deadline || (!isPast(new Date(t.deadline)) && !isToday(new Date(t.deadline))));
...
function TaskGroup({
    ...
    return (
        <Card className={cn(variant === 'destructive' && 'border-red-500/50')}>
            <CardHeader
                className={cn('cursor-pointer', collapsible && 'hover:bg-muted/50 transition-colors')}
...
function TaskDialog({
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, ...payload }: { id: string } & Partial<CreateTaskPayload>) =>
