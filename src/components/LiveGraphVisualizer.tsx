...
import { Brain, Wrench, CheckCircle2, AlertCircle, Loader2, Code2, Database, Search, FileText } from "lucide-react";
...
export const LiveGraphVisualizer: React.FC<LiveGraphVisualizerProps> = ({ workflowPhase, agents, isSynthesizing }) => {
  ...
  return (
    <div 
      ...
      style={{
        background: "radial-gradient(ellipse at 50% -20%, hsl(var(--primary)/0.03), transparent 70%)"
      }}
    >
      {/* Root Node */}
      <div className="flex flex-col items-center justify-start min-w-[500px]">
        <motion.div 
          initial={{ scale: 0 }} 
          animate={{ scale: 1 }} 
        ...
        >
          <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-muted/40 border border-primary/20 shadow-[0_0_20px_rgba(var(--primary-glow),0.1)] backdrop-blur">
            ...
            <span className="text-sm font-semibold tracking-wide uppercase text-foreground/80">Swarm Orchestrator</span>
            <span className="text-xs text-muted-foreground mt-1 capitalize">{workflowPhase.replace("_", " ")}</span>
          </div>
        </motion.div>
        ...
        <div className="relative flex justify-center gap-12 mt-0 pt-4">
          {/* Connector horizontal line for parallel agents */}
          {allAgents.length > 1 && (
            <motion.div 
              initial={{ width: 0 }} 
              animate={{ width: "100%" }} 
              className="absolute top-0 left-0 bg-border h-px w-full"
              style={{
                left: "10%",
                width: "80%"
              }}
            ...
            {allAgents.map((agent, index) => {
              ...
              return (
                <motion.div 
                  ...
                  className="flex flex-col items-center relative z-10 min-w-[180px]"
                >
                  ...
                  <div className={cn(
                    ...
                    isRunning && "border-primary shadow-[0_0_20px_rgba(var(--primary),0.2)]",
                    ...
                    <span className="text-sm font-semibold text-center mb-1 line-clamp-1">{agent.name}</span>
                    <span className="text-[10px] uppercase text-muted-foreground font-medium">{agent.role}</span>
                  </div>
                        ...
                        {agent.tools.map((tool) => (
                          ...
                          >
                            <div className="bg-background rounded p-1.5 shadow-sm">
                              {tool.status === "running" ? <Loader2 className="w-3.5 h-3.5 text-accent animate-spin" /> :
                               tool.status === "success" ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> :
                                                           <AlertCircle className="w-3.5 h-3.5 text-rose-500" /> }
                            </div>
                            <div className="flex flex-col flex-1 overflow-hidden">
                              <span className="text-xs font-medium truncate">{tool.name}</span>
                              {tool.time_ms && <span className="text-[9px] text-muted-foreground">{tool.time_ms}ms</span>}
                            </div>
                          </motion.div>
                        ))}
                      </AnimatePresence>
                  ...
                  {agent.output && (
                    <motion.div 
                      className="mt-6 p-3 rounded-lg bg-black/80 border border-white/10 w-full max-w-[260px] text-white/80 overflow-hidden relative shadow-inner"
                    >
                      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent" />
                      <div className="flex items-center gap-1.5 mb-2">
                        <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                        <span className="text-[9px] font-mono tracking-widest uppercase text-white/50">stdout</span>
                      </div>
        ...
        {allAgents.length > 0 && isSynthesizing && (
          ...
          >
            ...
            <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl shadow-[0_0_30px_rgba(245,158,11,0.1)] w-full text-center flex flex-col items-center">
              ...
              <span className="text-sm font-bold text-amber-600 dark:text-amber-400">Synthesizing Final Output</span>
              <span className="text-xs text-muted-foreground mt-1">Merging agent knowledge...</span>
            </div>
          </motion.div>
        )}
      </div>
