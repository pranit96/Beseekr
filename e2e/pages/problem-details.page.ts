...
export class ProblemDetailsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Score
    this.scoreCircle = page.locator(
      '[class*="rounded-full"][class*="border-4"]',
    );
    this.scoreValue = page.locator('[class*="text-3xl"][class*="font-bold"]');
    this.verdict = page
      .locator("h2")
      .filter({ hasText: /(Worth|Early|Needs|Strong)/i });
    this.confidenceBadge = page
      .locator('[class*="Badge"]')
      .filter({ hasText: /confidence/i });
