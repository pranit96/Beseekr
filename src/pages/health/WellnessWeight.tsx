...
export default function WellnessWeight() {
  ...
  const targetWeight =
    profile?.targetWeightKg ?? (profile as any)?.target_weight_kg;
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur-md">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-teal-400 uppercase">
                Body
              </p>
          ...
          <section className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {change !== null && (
              <div className="rounded-xl border border-border bg-muted/20 p-4 text-center">
                ...
                <p
                  className={`text-xl font-bold flex items-center justify-center gap-1 ${change < 0 ? "text-green-400" : change > 0 ? "text-rose-400" : "text-muted-foreground"}`}
                >
                  {change < 0 ? (
              ...
              >
                ...
                {targetWeight &&
                  ...
                  targetWeight <= maxW && (
                    <line
                      x1="0"
                      y1={100 - ((targetWeight - minW) / rangeW) * 100}
                      x2="100"
                      y2={100 - ((targetWeight - minW) / rangeW) * 100}
                      stroke="#14b8a6"
                      strokeWidth="0.5"
                      strokeDasharray="3 2"
                      opacity={0.7}
                ...
                {chartData.map((w, i) => {
                  ...
                  const y = 100 - ((Number(w.weight_kg) - minW) / rangeW) * 100;
                  return (
                    <circle key={i} cx={x} cy={y} r="1.2" fill="#14b8a6" />
                  );
                })}
              </svg>
              {/* Y-axis labels */}
                  ...
                  >
                    <div>
                      <p className="text-xs font-medium">
                        ...
                        {w.body_fat_pct
                          ? ` · ${Number(w.body_fat_pct).toFixed(1)}% BF`
                          : ""}
                      </p>
                    ...
                    <button
                      onClick={() => deleteWeight.mutateAsync(w.id)}
