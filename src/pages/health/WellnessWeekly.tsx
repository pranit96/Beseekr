...
export default function WellnessWeekly() {
  ...
  const ReviewSection = ({
    ...
    const colorClass =
      SECTION_COLORS[sectionKey] || "text-muted-foreground bg-muted";
      ...
      <div className="rounded-xl border border-border bg-background/50 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`h-8 w-8 rounded-lg flex items-center justify-center ${colorClass.split(" ")[1]}`}
            >
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-sm font-bold ${data.score >= 70 ? "text-green-400" : data.score >= 40 ? "text-amber-400" : "text-red-400"}`}
            >
              {data.score}/100
            </span>
            {isOpen ? (
        ...
        {isOpen && (
          <div className="px-4 pb-4 space-y-3 border-t border-border/50 pt-3">
            {data.highlights?.length > 0 && (
              <div>
                <p className="text-[10px] uppercase text-muted-foreground mb-1 font-semibold">
                  Highlights
                </p>
                ...
                <ul className="space-y-1">
                  {data.suggestions.map((s: string, i: number) => (
                    <li
                      ...
                      className="text-xs text-foreground/80 flex items-start gap-2"
                    >
                      <ArrowUpRight className="h-3 w-3 text-amber-400 mt-0.5 shrink-0" />{" "}
                      {s}
                    </li>
                  ))}
                </ul>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-4 sm:py-6 space-y-5">
        ...
        <section className="rounded-2xl border border-border bg-gradient-to-br from-primary/10 to-accent/5 p-5 text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground max-w-md mx-auto">
            Get a comprehensive review of your training, nutrition, mood, sleep
            and habits from the past 7 days — powered by AI.
          </p>
          <button
        ...
        {latestReview && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {["training", "nutrition", "mood", "sleep", "habits"].map(
                (key) => (
                  <ReviewSection
                    key={key}
                    sectionKey={key}
                    data={
                      latestReview.review?.[
                        key as keyof typeof latestReview.review
                      ]
                    }
                  />
                ),
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {latestReview.review?.next_week_priorities?.length > 0 && (
                <div className="rounded-xl border border-border bg-amber-500/5 p-4 space-y-2">
                  ...
                  <ul className="space-y-1">
                    {latestReview.review.next_week_priorities.map(
                      (p: string, i: number) => (
                        <li key={i} className="text-xs text-foreground/80">
                          → {p}
                        </li>
          ...
          <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-3">
            ...
            <div className="space-y-2">
              {reviews.slice(1).map((r) => (
                <div
                  key={r.id}
                  className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-background/50 border border-border/50"
                >
                  <div>
                  ...
                  <span
                    className={`text-sm font-bold ${(r.review?.overall_score ?? 0) >= 70 ? "text-green-400" : (r.review?.overall_score ?? 0) >= 40 ? "text-amber-400" : "text-red-400"}`}
                  >
