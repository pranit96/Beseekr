...
function KPICard({ label, value, icon: Icon, color, prefix }: {
  label: string; value: string | number; icon: any; color: string; prefix?: string;
}) {
  return (
    <motion.div variants={item}>
      <Card className="bg-card/50 backdrop-blur-sm border-border/30 relative overflow-hidden group hover:border-primary/20 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5">
        ...
        <CardContent className="p-4 pt-0">
          <span className={`text-xl font-bold tracking-tight ${color || "text-foreground"}`}>
            {prefix}{value}
          </span>
        </CardContent>
      </Card>
...
export default function BudgetOverview() {
  const { dashboard, loadingDashboard, yearlyOverview, preferredCurrencySymbol } = useBudget();

  const fmt = (n?: number) =>
    n !== undefined ? n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0.00";

  // Chart data
  const dailySpendingChartData = dashboard?.dailySpending
    ? Object.entries(dashboard.dailySpending).map(([date, amount]) => ({ date: date.substring(5), amount })).sort((a, b) => a.date.localeCompare(b.date))
    : [];

  const categoryBreakdownChartData = dashboard?.categoryBreakdown
    ? Object.entries(dashboard.categoryBreakdown).map(([name, val]) => ({ name, value: val.total }))
    : [];

  const yearlyOverviewChartData = yearlyOverview?.monthly
    ? Object.entries(yearlyOverview.monthly).map(([monthStr, data]) => ({ month: monthStr, income: data.income, expenses: data.expenses, savings: data.savings }))
    : [];
  ...
  return (
    <div className="space-y-6">
      {/* ── KPI Cards ── */}
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <KPICard label="Net Savings" value={fmt(netSavings)} icon={isPositive ? TrendingUp : TrendingDown} color={isPositive ? "text-emerald-500" : "text-rose-500"} prefix={preferredCurrencySymbol} />
        <KPICard label="Savings Rate" value={`${(dashboard?.summary.savingsRate ?? 0).toFixed(1)}%`} icon={Percent} color="text-blue-500" />
        <KPICard label="Total Income" value={fmt(dashboard?.summary.totalIncome)} icon={Plus} color="text-emerald-500" prefix={preferredCurrencySymbol} />
        <KPICard label="Total Expenses" value={fmt(dashboard?.summary.totalExpenses)} icon={TrendingDown} color="text-rose-500" prefix={preferredCurrencySymbol} />
        <KPICard label="Avg Daily" value={fmt(dashboard?.summary.avgDailySpend)} icon={Calendar} color="text-amber-500" prefix={preferredCurrencySymbol} />
        <KPICard label="Projected" value={fmt(dashboard?.summary.projectedMonthlySpend)} icon={ArrowRight} color="text-purple-500" prefix={preferredCurrencySymbol} />
      </motion.div>

      {/* ── Charts Row ── */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="grid lg:grid-cols-3 gap-4">
        {/* Daily Spending */}
        <Card className="lg:col-span-2 bg-card/50 backdrop-blur-sm border-border/30 rounded-2xl relative overflow-hidden">
          ...
          <CardHeader className="p-5 pb-3">
            ...
            <CardDescription className="text-xs text-muted-foreground">Aggregated daily expense flow for this period.</CardDescription>
          </CardHeader>
            ...
            {dailySpendingChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailySpendingChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{ backgroundColor: "hsl(var(--background))", borderColor: "hsl(var(--border))", borderRadius: "12px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.15)" }}
                    labelFormatter={(label) => `Date: ${label}`}
                    formatter={(val: any) => [`${preferredCurrencySymbol}${val.toFixed(2)}`, "Expenses"]}
                  />
                  <Area type="monotone" dataKey="amount" stroke="hsl(var(--primary))" strokeWidth={2} fillOpacity={1} fill="url(#spendGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card className="bg-card/50 backdrop-blur-sm border-border/30 rounded-2xl flex flex-col">
          <CardHeader className="p-5 pb-3">
            ...
            <CardDescription className="text-xs text-muted-foreground">Spending by category this period.</CardDescription>
          </CardHeader>
            ...
            {categoryBreakdownChartData.length === 0 ? (
              ...
              <div className="space-y-3.5">
                {categoryBreakdownChartData.map((cat) => {
                  ...
                  const percentage = Math.min((cat.value / progressMax) * 100, 100);
                  return (
                    <div key={cat.name} className="space-y-1">
                      <div className="flex justify-between text-xs font-medium">
                        ...
                        <span className="text-foreground font-semibold tabular-nums">
                          {preferredCurrencySymbol}{cat.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </span>
                      </div>
      ...
      {/* ── Bottom Row ── */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45, duration: 0.5 }} className="grid md:grid-cols-2 gap-4">
        {/* Top Merchants */}
        <Card className="bg-card/50 backdrop-blur-sm border-border/30 rounded-2xl">
          <CardHeader className="p-5 pb-3">
            ...
            <CardDescription className="text-xs text-muted-foreground">Where you spend the most.</CardDescription>
          </CardHeader>
          <CardContent className="px-5 pb-5">
            {(!dashboard?.topMerchants || dashboard.topMerchants.length === 0) ? (
              <div className="py-8 text-center text-muted-foreground text-sm">No merchants tracked.</div>
            ) : (
              <div className="divide-y divide-border/20">
                {dashboard.topMerchants.map((merchant, idx) => (
                  <div key={idx} className="flex justify-between py-3 items-center text-sm group">
                    <span className="font-medium text-foreground flex items-center gap-2.5">
                      ...
                      <span className="truncate">{merchant.name || "Unknown"}</span>
                    </span>
        ...
        <Card className="bg-card/50 backdrop-blur-sm border-border/30 rounded-2xl">
          <CardHeader className="p-5 pb-3">
            ...
            <CardDescription className="text-xs text-muted-foreground">Monthly income vs expenses year-to-date.</CardDescription>
          </CardHeader>
            ...
            {yearlyOverviewChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={yearlyOverviewChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="expenses" fill="hsl(0 84% 60%)" radius={[4, 4, 0, 0]} opacity={0.85} name="Expenses" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
