      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Analytics, progress & revision schedule
          </p>
        </div>
        {dueCount > 0 && (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-0">
                    ...
                    <div className="h-40 flex items-end gap-2">
                      {dashboard.score_trend?.slice(-15)?.map((point, i) => {
                        const maxScore = Math.max(
                          ...(dashboard.score_trend?.map((p) => p.score) || []),
                          200,
                        );
                        return (
                          <div
              ...
              {speedAnalytics?.topics?.length > 0 && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Avg time per topic vs target • Top{" "}
                      {speedAnalytics.percentile_speed}% speed
                    </CardDescription>
                  </CardHeader>
                      ...
                      <div key={topic.id} className="flex items-center gap-3">
                        <div className="flex-1">
                          <div className="flex justify-between mb-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {Math.round(topic.avg_time_seconds / 60)}m avg /{" "}
                              {Math.round(topic.target_time_seconds / 60)}m
                              target
                            </span>
                          </div>
                          <Progress
                            value={Math.min(
                              (topic.target_time_seconds /
                                Math.max(topic.avg_time_seconds, 1)) *
                                100,
                              100,
                            )}
                            className="h-2"
        ...
        <TabsContent value="revisions" className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card
              className={cn(grouped.overdue.length > 0 && "border-red-500/50")}
            >
              <CardContent className="pt-6 text-center">
            ...
            <Card
              className={cn(grouped.today.length > 0 && "border-amber-500/50")}
            >
              <CardContent className="pt-6 text-center">
              ...
              {studyPlan && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Week of {safeFormatDate(studyPlan.week_start, "MMM d")} -{" "}
                      {safeFormatDate(studyPlan.week_end, "MMM d")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <div key={i} className="p-3 rounded-lg bg-muted/50">
                          ...
                          <div className="space-y-1">
                            {day.tasks?.slice(0, 3)?.map((task, j) => (
                              <div
                                key={j}
                                className="text-xs p-1.5 rounded bg-background"
                              >
                                <Badge
                                  variant="outline"
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    {safeFormatDate(r.scheduled_date, "MMM d")}
                  </p>
                </div>
                {r.status !== "completed" && (
