    ...
    <HiredShell>
      <div className="max-w-6xl mx-auto space-y-12 py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tighter leading-tight text-zinc-900 dark:text-white">
        ...
        {!hasResume ? (
          <div className="flex items-center gap-3 bg-amber-50 dark:bg-amber-500/[0.06] border border-amber-200 dark:border-amber-500/20 rounded-2xl px-5 py-3.5">
            ...
            <button
              ...
              className="text-[11px] font-black uppercase tracking-widest text-amber-600 hover:text-amber-700 dark:text-amber-400 dark:hover:text-amber-300 transition-colors whitespace-nowrap"
            >
              Load Resume →
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-500/[0.04] border border-emerald-200 dark:border-emerald-500/15 rounded-2xl px-5 py-3">
            <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-500/70 shrink-0" />
            <p className="text-xs text-emerald-800 dark:text-emerald-400/70 font-bold">
            ...
            return (
              <motion.div
                ...
                className={`group relative border rounded-[28px] p-7 backdrop-blur-3xl cursor-pointer transition-all duration-300 ${
                  locked
                    ? "border-zinc-200/60 dark:border-white/[0.05] bg-zinc-100/30 dark:bg-white/[0.01] opacity-60"
                    : "border-zinc-200 dark:border-white/[0.07] hover:border-zinc-300 dark:hover:border-white/[0.14] bg-white dark:bg-white/[0.02] shadow-sm hover:shadow-md"
                }`}
              >
                <div
                ...
                <div className="relative space-y-5">
                  <div className="w-12 h-12 rounded-2xl bg-zinc-50 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/[0.08] flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:border-zinc-300 dark:group-hover:border-white/20">
                    {feature.icon}
                  </div>
                  <div className="space-y-1.5">
                    <h3 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
                      {feature.title}
                      {locked ? (
                        <span className="text-[10px] font-black text-amber-600 dark:text-amber-500/70 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-full px-2 py-0.5 uppercase tracking-wider">
                          Needs Resume
                        </span>
                      ) : (
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {feature.badges.map((b) => (
                      <Badge
                        key={b}
                        className="bg-zinc-100 dark:bg-white/[0.04] text-zinc-600 dark:text-zinc-400 border-transparent text-[10px] font-bold px-2.5 py-0.5 rounded-lg"
                      >
                        {b}
                      </Badge>
                    ))}
