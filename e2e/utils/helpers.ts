...
export async function interceptApiRequests(page: Page, urlPatterns: (string | RegExp)[]): Promise<Map<string, any[]>> {
...
export async function expectElementCount(page: Page, selector: string, count: number): Promise<void> {
    const elements = page.locator(selector);
    await expect(elements).toHaveCount(count);
}

// Assert element visible with timeout
export async function expectVisibleWithTimeout(page: Page, selector: string, timeout = 10000): Promise<void> {
