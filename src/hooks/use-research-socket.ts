...
function getPendingJobs(): Map<
  string,
  { problemId: string; startedAt: number }
> {
  try {
...
function savePendingJobs(
  jobs: Map<string, { problemId: string; startedAt: number }>,
) {
...
export function useResearchSocket(
  options: UseResearchSocketOptions = {},
): UseResearchSocketReturn {
  const { autoNavigate = true, showToasts = true, problemId } = options;
  const navigate = useNavigate();

  const [isConnected, setIsConnected] = useState(false);
  const [lastEvent, setLastEvent] = useState<
    ResearchCompleteEvent | ResearchFailedEvent | null
  >(null);
  ...
  const handleResearchFailed = useCallback(
    (data: ResearchFailedEvent) => {
