...
export const AgentQuickChat: React.FC<AgentQuickChatProps> = ({ agent, open, onClose }) => {
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
                ...
                },
                {
                    ...
                    onAgentError: (_agentId: string, error: any) => {
                        setMessages(prev =>
                            prev.map(m =>
                                m.id === assistantMsgId
                                    ? { ...m, content: String(error || 'Error generating response'), status: 'error' }
                                    : m
                            )
                        );
                    },
                    ...
                    onError: (err: any) => {
                        setMessages(prev =>
                            prev.map(m =>
                                m.id === assistantMsgId
                                    ? { ...m, content: err?.error || 'Request failed', status: 'error' }
                                    : m
                            )
                        );
                    },
            ...
            setMessages(prev =>
                prev.map(m =>
                    m.id === assistantMsgId
                        ? { ...m, content: err?.message || 'Failed to send message', status: 'error' }
                        : m
                )
            );
        } finally {
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-border/60">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">{agent.domain || 'General'} · Quick Chat</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                    {messages.length === 0 && (
                        <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-12">
                            ...
                            <div>
                                <h4 className="font-semibold text-sm mb-1">Chat with {agent.name}</h4>
                                <p className="text-xs text-muted-foreground max-w-[280px]">
                                    Send a message to test this agent. Messages are not saved to history.
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="px-4 py-3 border-t border-border/60 bg-background/95">
                    ...
                    {isExecuting && (
                        <div className="text-xs text-muted-foreground text-center mt-2 flex items-center justify-center gap-2">
                            ...
                            Processing with <span className="text-primary font-medium">{agent.name}</span>
                            {elapsedTime > 0 && <span className="text-muted-foreground/60">· {elapsedTime}s</span>}
                        </div>
                    )}
                </div>
            </div>
