...
import { ShieldCheck, ChevronDown, ChevronUp, CheckCircle2, XCircle, RefreshCcw, Cpu } from "lucide-react";
import { useMyAgents } from "@/hooks/use-api-queries";

const CHECK_MODES = [
  { value: "min_length", label: "Min Character Length", placeholder: "500" },
  { value: "no_error_strings", label: "No Error Strings", placeholder: "(no config needed)" },
  { value: "regex_match", label: "Regex Match", placeholder: "\\d{4}-\\d{2}-\\d{2}" },
  { value: "ai_judge", label: "AI Quality Judge", placeholder: "Must be professional and include 3+ action items" },
];
...
const RetryNode = memo(({ data, selected }: NodeProps) => {
  ...
  const currentMode = CHECK_MODES.find(m => m.value === checkMode) || CHECK_MODES[0];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 pt-3 pb-2.5 border-b border-emerald-500/15">
        ...
        `}>
          {isDone && passed ? <CheckCircle2 className="w-4 h-4 text-white" /> :
           isDone && !passed ? <XCircle className="w-4 h-4 text-white" /> :
           isRunning ? <RefreshCcw className="w-4 h-4 text-white animate-spin" /> :
           <ShieldCheck className="w-4 h-4 text-white" />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Quality Gate</div>
      ...
      <div className="px-3.5 py-3 space-y-2.5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-black text-emerald-400/60 uppercase tracking-widest">Retry Budget</span>
            {attemptsUsed > 0 && (
              <span className={`text-[10px] font-black ${passed ? "text-emerald-400" : "text-rose-400"}`}>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-emerald-500/8 border border-emerald-500/15">
          ...
          <span className="text-[11px] font-semibold text-emerald-300/80 truncate">{agentName}</span>
        </div>

        {/* Check mode badge */}
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-emerald-500/8 border border-emerald-500/15">
          <ShieldCheck className="w-3 h-3 text-emerald-400 shrink-0" />
          <span className="text-[11px] font-semibold text-emerald-300/80 truncate">{currentMode.label}</span>
        </div>
        ...
        {expanded && (
          <div className="space-y-2 pt-1 border-t border-emerald-500/10">
            <div className="space-y-1">
              <label className="text-[9px] font-black text-emerald-400/50 uppercase tracking-widest">Target Agent</label>
              <select
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-black text-emerald-400/50 uppercase tracking-widest">Max Retries</label>
              <input
                type="number"
                min={0}
                max={3}
                value={maxRetries}
                onChange={e => (data.onMaxRetriesChange as Function)?.(parseInt(e.target.value))}
                className="w-full px-2.5 py-1.5 rounded-lg text-xs bg-background/40 border border-emerald-500/20 text-foreground focus:border-emerald-400/50 focus:outline-none"
              />
            </div>
            <div className="space-y-1">
              <label className="text-[9px] font-black text-emerald-400/50 uppercase tracking-widest">Quality Check Mode</label>
              <select
                value={checkMode}
                onChange={e => (data.onCheckModeChange as Function)?.(e.target.value)}
                className="w-full px-2.5 py-1.5 rounded-lg text-xs bg-background/40 border border-emerald-500/20 text-foreground focus:border-emerald-400/50 focus:outline-none"
              >
                {CHECK_MODES.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
            </div>
            {checkMode !== "no_error_strings" && (
              <div className="space-y-1">
                <label className="text-[9px] font-black text-emerald-400/50 uppercase tracking-widest">
                  {checkMode === "min_length" ? "Min Characters" :
                   checkMode === "regex_match" ? "Regex Pattern" :
                   "Quality Criteria"}
                </label>
                <input
                  value={checkValue}
                  onChange={e => (data.onCheckValueChange as Function)?.(e.target.value)}
