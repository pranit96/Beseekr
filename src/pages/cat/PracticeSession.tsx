...
export default function PracticeSession({ session, questions, onComplete, onExit }: PracticeSessionProps) {
    const [currentIndex, setCurrentIndex] = useState(0);
    const [answers, setAnswers] = useState<Record<string, string>>({});
    const [reviewMode, setReviewMode] = useState(false);
    const [timeLeft, setTimeLeft] = useState(session.time_limit_seconds || 0);
    const [isPaused, setIsPaused] = useState(false);
    const [results, setResults] = useState<Record<string, ProblemAttemptResponse>>({});
                ...
                <CardContent className="py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            disabled={submitting}
                        >
                            {isLastQuestion ? 'Submit' : 'Next'} <ChevronRight className="h-4 w-4 ml-1" />
                        </Button>
                    </div>
                </CardContent>
            </Card>
                    ...
                    <Card className="min-h-[400px] flex flex-col">
                        <CardHeader>
                            ...
                            <CardTitle className="text-xl leading-relaxed">
                                {currentQuestion.question_text || (currentQuestion as any).content}
                            </CardTitle>
                        </CardHeader>
