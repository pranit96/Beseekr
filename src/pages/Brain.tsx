...
export default function Brain() {
  ...
  useEffect(() => {
    if (viewTab === "chat") setTimeout(() => inputRef.current?.focus(), 100);
  }, [viewTab]);
  ...
  return (
    <div className="h-screen flex flex-col bg-[#090b12] text-foreground overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-0">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full bg-purple-800/[0.03] blur-[100px]" />
      </div>
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <aside className="w-56 shrink-0 flex flex-col border-r border-white/[0.05] bg-[#0c0e18]/80 backdrop-blur-sm p-3 gap-1">
          ...
          <nav className="flex flex-col gap-0.5">
            {TAB_CONFIG.map((tab) => (
              <button
                ...
                onClick={() => {
                  setViewTab(tab.id);
                  if (tab.id === "map" && nodes.length === 0) generateMap();
                  if (tab.id === "insights" && !insights) generateInsights();
                }}
          ...
          <div className="mt-auto pt-3 border-t border-white/[0.05]">
            ...
            {/* Library count badge if items exist */}
            {items.length > 0 && (
                            ...
                            <div className="max-w-[75%] flex flex-col gap-2">
                              <div
                                className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                                  msg.role === "user"
                                    ? "bg-violet-600 text-white rounded-br-md ml-auto shadow-lg shadow-violet-500/20"
      ...
      {/* ── Add Context Panel (Full-screen Overlay) ──────── */}
      <AnimatePresence>
        {showAddPanel && (
          <>
            <motion.div
