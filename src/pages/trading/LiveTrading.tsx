    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">Real-time signals with live price updates</p>
        </div>
        <div className="flex items-center gap-2">
          <Activity className={cn('h-5 w-5', wsState.connected ? 'text-green-500' : 'text-red-500')} />
          <span className={cn('text-sm', wsState.connected ? 'text-green-500' : 'text-red-500')}>
      ...
      {signals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {signals.map((signal) => {
            ...
            const priceChange = priceData ? ((currentPrice - signal.entry_price) / signal.entry_price * 100) : 0;
              ...
              <div key={signal.id} className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4 text-sm">
                  <div>
                    ...
                    <div className="text-green-400 font-medium">₹{signal.target_price.toFixed(2)}</div>
                  </div>
                  <div>
                    <div className="text-slate-500">Stop Loss</div>
                    <div className="text-red-400 font-medium">₹{signal.stop_loss.toFixed(2)}</div>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                  <div className="text-sm">
                    ...
                    <span className="text-white font-medium">{signal.confidence_score}%</span>
                  </div>
                  <div className="text-sm">
                    <span className="text-slate-500">Strategy: </span>
                    <span className="text-white font-medium">{signal.trading_strategies.name}</span>
                  </div>
                </div>
              </div>
            );
