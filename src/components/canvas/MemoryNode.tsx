...
import { Brain, ChevronDown, ChevronUp, Eye, EyeOff, Database } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 pt-3 pb-2.5 border-b border-violet-500/15">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-black text-violet-400 uppercase tracking-widest">Memory</div>
      ...
      <div className="px-3.5 py-3 space-y-2.5">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-violet-500/8 border border-violet-500/15">
          ...
          <code className="text-[11px] font-mono font-bold text-violet-300/90 truncate flex-1">{memoryKey}</code>
          {byteSize > 0 && (
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isDone && (
            <span className="ml-auto px-2 py-0.5 rounded-md text-[10px] font-black bg-emerald-500/15 border border-emerald-500/25 text-emerald-400">✓</span>
          )}
        </div>
            ...
            {showPreview && (
              <div className="px-2.5 py-2 rounded-xl bg-violet-500/5 border border-violet-500/15 max-h-24 overflow-y-auto">
                <pre className="text-[10px] text-foreground/70 whitespace-pre-wrap leading-relaxed font-mono">
                  {memoryPreview.length > 400 ? memoryPreview.slice(0, 400) + "…" : memoryPreview}
                </pre>
              </div>
            )}
            {lastUpdated && (
        ...
        {expanded && (
          <div className="space-y-2 pt-1 border-t border-violet-500/10">
            <div className="space-y-1">
              <label className="text-[9px] font-black text-violet-400/50 uppercase tracking-widest">Memory Key</label>
              <input
                value={memoryKey}
                onChange={e => (data.onMemoryKeyChange as Function)?.(e.target.value)}
              ...
              />
              <p className="text-[9px] text-muted-foreground/40 leading-relaxed">Shared across all runs of this workflow. Isolated per user.</p>
            </div>
            <div className="space-y-1">
              <label className="text-[9px] font-black text-violet-400/50 uppercase tracking-widest">Operation</label>
              <select
                value={operation}
                onChange={e => (data.onOperationChange as Function)?.(e.target.value)}
              ...
              >
                {MEMORY_OPS.map(op => <option key={op.value} value={op.value}>{op.label}</option>)}
              </select>
            </div>
          </div>
        )}
