...
export const WorkflowBuilder: React.FC<WorkflowBuilderProps> = ({
    ...
    const availableAgents = agents.filter(a => !nodes.some(n => n.agentId === a.id));
            ...
            <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-5 pt-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="wf-desc" className="text-xs">Description (optional)</Label>
                            <Input
                                id="wf-desc"
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {availableAgents.length > 0 ? (
                                ...
                                <p className="text-xs text-muted-foreground">All agents added to workflow</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <Label className="text-xs">Execution Chain ({nodes.length} steps)</Label>
                            {nodes.length > 0 && (
                        ...
                        {nodes.length === 0 ? (
                            <div className="border-2 border-dashed border-border/50 rounded-xl p-8 text-center">
                                ...
                                <p className="text-sm text-muted-foreground">Click agents above to add them to the chain</p>
                                <p className="text-xs text-muted-foreground/60 mt-1">Drag to reorder · Click to expand instructions</p>
                            </div>
                        ) : (
                                ...
                                {nodes.map((node, index) => (
                                    <React.Fragment key={node.id}>
                                        ...
                                        <div
                                            ...
                                            className={`group relative flex items-center gap-3 p-3 rounded-xl border transition-all cursor-grab active:cursor-grabbing ${draggedIndex === index
                                        ...
                                        >
                                            ...
                                            {/* Step number */}
                                            <div className={`w-7 h-7 rounded-full ${node.agentColor} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}>
                                                {index + 1}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-semibold truncate">{node.agentName}</span>
                                                ...
                                                {expandedNode === node.id && (
                                                    <div className="mt-2">
                                                        <Input
                                                            ...
                                                            onChange={e => updateNodeInstruction(node.id, e.target.value)}
