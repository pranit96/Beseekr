...
export default function BudgetInsights() {
  ...
  const expChange =
    insights?.month_over_month?.expense_change_pct ??
    insights?.month_over_month?.expenseChange;
      ...
      if (response.success) {
        if (response.data) {
          ...
          toast({
            ...
            description: "AI financial diagnostics refreshed.",
          });
        } else {
          toast({
            title: "No Data Available",
            description:
              "No transactions found for the selected month. Please add or import transactions first.",
          });
        }
      }
    } catch (e: any) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 sm:p-6 border-b border-border/20">
          <div>
            <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-indigo-500" /> Financial
              Insights
            </h3>
            <p className="text-sm text-muted-foreground mt-0.5">
              AI-generated highlights, anomaly detection, and actionable
              recommendations.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center border border-dashed border-border/40 rounded-2xl flex flex-col items-center justify-center bg-muted/5">
              ...
              <p className="text-sm text-muted-foreground max-w-sm mb-6">
                Click the button above to run AI diagnostics over your spending
                history.
              </p>
              <Button
                onClick={handleGenerate}
                variant="outline"
                size="sm"
                className="gap-1.5 rounded-xl border-indigo-500/20 text-indigo-500 hover:bg-indigo-500/10"
              >
                  ...
                  <div className="relative w-18 h-18 shrink-0 flex items-center justify-center">
                    <svg className="w-16 h-16 transform -rotate-90">
                      ...
                      <circle
                        ...
                        strokeDashoffset={
                          2 * Math.PI * 26 -
                          ((insights.financial_health_score ?? 50) / 100) *
                            2 *
                            Math.PI *
                            26
                        }
                ...
                <Card className="bg-background/60 border-border/20 p-5 rounded-xl flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {insights.month_over_month?.previousExpenses !==
                        undefined &&
                        insights.month_over_month.previousExpenses !== null && (
                          <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-border/10 pt-1.5 mt-1.5">
                            ...
                            <span className="font-semibold">
                              {preferredCurrencySymbol}
                              {insights.month_over_month.previousExpenses.toLocaleString()}
                            </span>
                          </div>
                        )}

                      {insights.month_over_month?.previousIncome !==
                        undefined &&
                        insights.month_over_month.previousIncome !== null && (
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <span>Prev. Month Income:</span>
                            <span className="font-semibold">
                              {preferredCurrencySymbol}
                              {insights.month_over_month.previousIncome.toLocaleString()}
                            </span>
                          </div>
                        )}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-semibold text-foreground">
                          {preferredCurrencySymbol}
                          {(insights.total_expenses ?? 0).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">
                          {preferredCurrencySymbol}
                          {(insights.savings_amount ?? 0).toLocaleString()}
                        </span>
                      </div>
                ...
                <Card className="bg-background/60 border-border/20 rounded-xl p-5 relative overflow-hidden md:col-span-2 flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-relaxed text-muted-foreground">
                      {insights.ai_summary ||
                        "AI Summary text is being processed."}
                    </p>
                  </div>
                </Card>
                ...
                <Card className="bg-background/60 border-border/20 rounded-xl p-5 relative overflow-hidden flex flex-col justify-between">
                  <div>
                    <h4 className="text-sm font-semibold text-indigo-500 mb-3 flex items-center gap-1.5">
                      <Coins className="w-4 h-4 text-indigo-500" /> Category
                      Breakdown
                    </h4>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-emerald-500" /> Saving
                    Tips
                  </h4>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-blue-500" /> Spending
                    Patterns
                  </h4>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                    <AlertTriangle className="w-4 h-4 text-rose-500" /> Anomaly
                    Warnings
                  </h4>
