...
import { measureWebVitals, checkPerformanceThresholds, formatMetrics } from '../../utils/performance';
import { ROUTES, PERFORMANCE_THRESHOLDS } from '../../fixtures/test-data';

test.describe('Page Load Performance', () => {
    // Store metrics for report
    const performanceResults: Array<{ page: string; metrics: any; passed: boolean }> = [];
    ...
    test.afterAll(async () => {
        ...
        for (const result of performanceResults) {
            ...
            console.log(`   Load: ${result.metrics.pageLoadTime?.toFixed(0)}ms | TTI: ${result.metrics.timeToInteractive?.toFixed(0)}ms`);
        }
...
test.describe('Core Web Vitals', () => {
    test('LCP should be under threshold on problems page', async ({ page }) => {
        ...
        console.log(`📊 LCP: ${metrics.largestContentfulPaint?.toFixed(0)} ms(threshold: ${PERFORMANCE_THRESHOLDS.lcp}ms)`);
    ...
    test('Time to Interactive should be under threshold', async ({ page }) => {
        ...
        console.log(`📊 TTI: ${metrics.timeToInteractive?.toFixed(0)} ms(threshold: ${PERFORMANCE_THRESHOLDS.tti}ms)`);
