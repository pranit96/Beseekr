...
function isValidLogEntry(entry: any): entry is LogEntry {
  ...
  if (
    ...
    !entry.source ||
    entry.source.length > CONFIG.MAX_SOURCE_LENGTH
  ) {
    return false;
  }
  ...
  if (
    ...
    !entry.message ||
    entry.message.length > CONFIG.MAX_MESSAGE_LENGTH
  ) {
    return false;
  }
...
function sanitizeLogEntry(entry: LogEntry): LogEntry {
  ...
  if (entry.userAgent) {
    sanitized.userAgent = sanitizeString(
      entry.userAgent,
      CONFIG.MAX_USER_AGENT_LENGTH,
    );
  }
...
function checkRateLimit(clientId: string): {
  ...
  return {
    allowed: true,
    remaining: CONFIG.RATE_LIMIT_MAX_REQUESTS - current.count,
  };
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Log ALL levels in both development and production
    // We need visibility into every log in production
    const isDev = environment === "development";

    console.log(`\n📝 Logging all ${enrichedLogs.length} entries:\n`);
