...
export default function MockTest() {
  ...
  const [answers, setAnswers] = useState<
    Record<string, { answer: string; time: number }>
  >({});
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Mock completed!" });
      queryClient.invalidateQueries({ queryKey: ["cat-mock", id] });
    },
  });
  ...
  const handleAnswer = useCallback(
    (answer: string) => {
      ...
      if (currentQuestion) {
        setAnswers((prev) => ({
          ...prev,
          [currentQuestion.id]: { answer, time: timeSpent },
        }));
      }
    },
  ...
  const handleNext = () => {
    if (currentQuestion && selectedAnswer) {
      submitMutation.mutate({
        ...
        time: answers[currentQuestion.id]?.time || 0,
      });
    }
    if (mock && currentQ < mock.questions.length - 1) {
      setCurrentQ(currentQ + 1);
      setSelectedAnswer(
        answers[mock.questions[currentQ + 1]?.id]?.answer || null,
      );
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Badge>
            {currentQ + 1} / {mock.total_questions}
          </Badge>
        </div>
      </div>
      ...
      <div className="flex gap-1 flex-wrap">
        {mock.questions.map((q, i) => (
          <button
            ...
            onClick={() => {
              setCurrentQ(i);
              setSelectedAnswer(answers[q.id]?.answer || null);
            }}
            className={cn(
      ...
      <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You've answered {answered} of {mock.total_questions} questions.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card>
        <CardContent className="pt-6 text-center">
          ...
          <Progress
            value={scorePercent}
            className="h-3 max-w-xs mx-auto mb-4"
          />
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <p className="text-2xl font-semibold text-red-500">
                {mock.attempted - mock.correct}
              </p>
              <p className="text-sm text-muted-foreground">Wrong</p>
            </div>
            <div>
              <p className="text-2xl font-semibold text-muted-foreground">
                {mock.total_questions - mock.attempted}
              </p>
      ...
      {mock.section_scores && (
        <Card>
          ...
          <CardContent className="space-y-4">
            {Object.entries(mock.section_scores).map(
