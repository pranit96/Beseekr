  ...
  return (
    <div className="flex flex-col gap-0.5 px-3 py-2 rounded-xl bg-muted/20 border border-border/20">
      ...
      <div className="flex items-baseline gap-1">
        ...
        <div
          className={`flex items-center gap-0.5 ${color} text-[10px] font-bold`}
        >
...
export const ExecutionDiffPanel = memo(
  ({
    ...
    const unchangedCount = diffLines.filter(
      (l) => l.type === "unchanged",
    ).length;
            ...
            <div>
              ...
              {previousRun && (
                <p className="text-[10px] text-muted-foreground">
                  vs.{" "}
                  {new Date(previousRun.timestamp).toLocaleString(undefined, {
                    month: "short",
                    day: "numeric",
                    hour: "2-digit",
                    minute: "2-digit",
              ...
              {!previousRun ? (
                <div className="px-4 py-6 text-center text-muted-foreground/60 text-sm">
                  ...
                  <span className="text-[11px]">
                    Run the workflow again to see the diff.
                  </span>
                </div>
                ...
                visibleLines.map((line, i) => (
                  ...
                  >
                    <span
                      className={`select-none shrink-0 w-3 font-black ${
                        line.type === "added"
                          ? "text-emerald-500"
                          : line.type === "removed"
                            ? "text-rose-500"
            ...
            <div className="px-4 py-3 space-y-3">
              {!previousRun ? (
                <p className="text-sm text-muted-foreground/60 text-center py-6">
                  No previous run data available.
                </p>
              ) : (
                <>
                  ...
                  <div className="px-3 py-2.5 rounded-xl bg-muted/15 border border-border/20 space-y-1">
                    <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                      ...
                      <span>
                        {new Date(previousRun.timestamp).toLocaleString()}
                      </span>
                      <span
                        className={`ml-auto font-black ${previousRun.status === "success" ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {previousRun.status}
                      </span>
                    </div>
