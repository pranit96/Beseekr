...
test.describe("Problem Details Page", () => {
  ...
  test("should show watchlist button", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const watchVisible = await detailsPage.watchButton
        .isVisible()
        .catch(() => false);
      const watchingVisible = await detailsPage.watchingButton
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Error Handling", () => {
    test("should handle invalid problem ID", async ({ page }) => {
      ...
      const hasError = await page
        .locator(
          '[class*="error"], [class*="destructive"], text=/error|not found/i',
        )
        .first()
        .isVisible()
        .catch(() => false);
      const redirected =
        page.url().includes("/problems") && !page.url().includes("invalid");
