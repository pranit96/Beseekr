...
test.describe("Safari Mobile OAuth", () => {
  ...
  test("should display Google button on mobile Safari viewport", async ({
    page,
  }) => {
  ...
  test("should show configuration error if Supabase not configured", async ({
    page,
  }) => {
  ...
  test("should handle localStorage for session persistence", async ({
    page,
  }) => {
  ...
  test("should log Safari detection info", async ({ page }) => {
    ...
    const hasOAuthLog = logs.some((log) =>
      log.includes("Initiating Google OAuth"),
    );
