      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Track scores from IMS, TIME, and other platforms
          </p>
        </div>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-emerald-500">
                {analytics.average_percentile.toFixed(1)}%ile
              </p>
                ...
                <div className="h-16 flex items-end gap-1">
                  {analytics.trend.slice(-10).map((p, i) => (
                    <div
                      ...
                      style={{ height: `${p.percentile}%` }}
                      title={`${p.date}: ${p.percentile}%`}
                    />
                  ))}
                </div>
              )}
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="font-semibold">
                        {mock.overall_score}/{mock.max_score}
                      </p>
                      {mock.percentile && (
...
function ExternalMockDialog({
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      id,
      ...p
    }: { id: string } & Partial<CreateExternalMockPayload>) =>
      catApi.updateExternalMock(id, p),
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mock
      ? updateMutation.mutate({ id: mock.id, ...payload })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {mock ? "Edit Score" : "Add External Mock Score"}
          </DialogTitle>
          <DialogDescription>
            Track your performance from external platforms
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={platform}
                onValueChange={(v) => setPlatform(v as ExternalPlatform)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
