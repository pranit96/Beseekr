...
const sessionTypes: {
...
}[] = [
  {
    ...
    desc: "Standard practice with time limit",
    icon: Timer,
  },
  {
    value: "untimed",
    label: "Untimed",
    desc: "Take your time, no pressure",
    icon: Clock,
  },
  {
    value: "speed_drill",
    label: "Speed Drill",
    desc: "Fast-paced to improve speed",
    icon: Zap,
  },
  {
    value: "accuracy_focus",
...
export default function Practice() {
  ...
  const totalQuestions = history.reduce(
    (sum, s) => sum + s.questions_attempted,
    0,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Practice, review mistakes & saved questions
          </p>
        </div>
        <Button
      ...
      <SectionTabs
        tabs={[
          ...
          },
          {
            value: "mistakes",
            label: "Mistakes",
            description: "Review & learn from errors",
            icon: AlertCircle,
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-0">
                    ...
                    const accuracy =
                      session.questions_attempted > 0
                        ? (session.correct_answers /
                          ...
                          100
                        : 0;
                    const typeInfo = sessionTypes.find(
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium">
                              {typeInfo?.label || session.session_type}
                            </p>
        ...
        <TabsContent value="mistakes" className="space-y-6">
          ...
          <div className="grid grid-cols-5 gap-2">
            {(
              Object.entries(mistakeTypes) as [
                MistakeType,
                (typeof mistakeTypes)[MistakeType],
              ][]
            ).map(([type, info]) => {
              const count = mistakes.filter(
                (m) => m.mistake_type === type,
              ).length;
              return (
                <Card key={type} className="text-center">
                  <CardContent className="pt-4">
                    <div
                      className={cn(
                        "h-3 w-3 rounded-full mx-auto mb-2",
                        info.color,
                      )}
                    />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Click to review and learn from your errors
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isMistakesLoading ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium">
                              {mistake.question_text?.slice(0, 60)}...
                            </p>
                            <p className="text-sm text-muted-foreground">
                              {typeInfo?.label} •{" "}
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">
                    No unreviewed mistakes!
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Keep practicing to identify areas to improve
                  </p>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              {isBookmarksLoading ? (
                  ...
                  {bookmarks.map((bookmark: BookmarkType) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-medium">
                            {bookmark.question_text?.slice(0, 60)}...
                          </p>
                          <p className="text-sm text-muted-foreground">
                            {bookmark.collection_name || "Saved"} •{" "}
                      ...
                      <div className="flex items-center gap-2">
                        {bookmark.importance && (
                          <div className="flex">
                            {Array.from({
                              length:
                                bookmark.importance === "must_revise"
                                  ? 4
                                  : bookmark.importance === "high"
                                    ? 3
                                    : bookmark.importance === "medium"
                                      ? 2
                                      : 1,
                            }).map((_, i) => (
                              <Star
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            removeBookmarkMutation.mutate(bookmark.question_id)
                          }
                        >
                          Remove
                        </Button>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Bookmark questions during practice to review later
                  </p>
                </div>
              )}
            </CardContent>
      ...
      <ReviewMistakeDialog
        ...
        onSubmit={(notes) =>
          reviewMistake &&
          reviewMutation.mutate({ id: reviewMistake.id, notes })
        }
...
function StartPracticeDialog({
  ...
  const mutation = useMutation({
    mutationFn: (payload: StartPracticePayload) =>
      catApi.startPractice(payload),
  ...
  const handleStart = () => {
    mutation.mutate({
      ...
      time_limit_minutes:
        sessionType === "timed" ? parseInt(timeLimit) : undefined,
    });
  };
...
function ReviewMistakeDialog({
  ...
  const explainMutation = useMutation({
    ...
    onError: () =>
      toast({ title: "Failed to get explanation", variant: "destructive" }),
  });
              ...
              <div className="p-4 rounded-lg bg-muted/50">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <Badge
                    variant="outline"
                    className={cn(
                      mistakeTypes[mistake.mistake_type]?.color.replace(
                        "bg-",
                        "border-",
                      ),
                      "text-foreground",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select a topic to practice specific problems
        </CardDescription>
      </CardHeader>
...
function ProblemAttemptDialog({
  ...
  const submitMutation = useMutation({
    mutationFn: (payload: ProblemAttemptPayload) =>
      catApi.submitProblemAttempt(problem!.id, payload),
    onSuccess: (data) => {
      setResult(data);
      setShowResult(true);
      toast({
  ...
  const reportMutation = useMutation({
    mutationFn: () =>
      catApi.reportQuestion(problem!.id, {
          ...
          {!showResult ? (
            <>
              ...
              <Button
                ...
                disabled={submitMutation.isPending || !selectedAnswer}
              >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Today: {timerStats?.today_minutes || 0} min | Week:{" "}
                {timerStats?.this_week_minutes || 0} min
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
