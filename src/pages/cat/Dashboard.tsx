...
export default function CatDashboard() {
  ...
  const tasks = useMemo(() => {
    ...
    return items.filter(
      (t: StudyTask) => t.status !== "completed" && t.status !== "cancelled",
    );
  }, [tasksData]);

  const overdueTasks = useMemo(() => {
    return tasks.filter(
      (t: StudyTask) => t.deadline && new Date(t.deadline) < new Date(),
    );
  }, [tasks]);
  ...
  const overallProgress = parseFloat(
    dashboard?.syllabus?.progress_percent || "0",
  );
        ...
        <Card className="md:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              ...
              <CardDescription>
                {todayTasks.length} tasks for today, {overdueTasks.length}{" "}
                overdue
              </CardDescription>
            </div>
            <Link to="/cat/tasks">
            ...
            <ScrollArea className="h-[280px]">
              {tasks.length === 0 ? (
                ...
                <div className="space-y-2">
                  ...
                  {todayTasks
                    .filter((t: StudyTask) => !overdueTasks.includes(t))
                    .map((task: StudyTask) => (
                      <TaskItem
                        key={task.id}
                        task={task}
                  ...
                  {tasks
                    .filter(
                      (t: StudyTask) =>
                        !todayTasks.includes(t) && !overdueTasks.includes(t),
                    )
                    .slice(0, 3)
                    .map((task: StudyTask) => (
                      <TaskItem
          ...
          <CardContent className="space-y-4">
            {aiAnalysis ? (
              <>
                ...
                <div className="p-3 rounded-lg bg-violet-500/10 border border-violet-500/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {aiAnalysis.improvement_roadmap?.[0] ||
                ...
                {aiStudyPlan && (
                  <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                    ...
                    {aiStudyPlan.daily_plans?.[0]?.focus_areas
                      ?.slice(0, 2)
                      .map((area, i) => (
                        <Badge
                          key={i}
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {aiStudyPlan.daily_plans?.[0]?.estimated_hours || 0} hours
                      planned
                    </p>
                  </div>
                )}
                ...
                {tutorUsage && (
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>
                      AI Tutor: {tutorUsage.questions_today} queries today •{" "}
                      {tutorUsage.streak_days} day streak
                    </span>
                  </div>
                )}
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">
                  Complete more practice to unlock AI insights
                </p>
              </div>
            )}
          </CardContent>
          ...
          <CardContent>
            ...
            <div className="space-y-3">
              {dashboard?.syllabus?.subjects?.map((subject, idx) => (
                <Link key={idx} to="/cat/learn" className="block">
                  <motion.div
                    whileHover={{ x: 4 }}
            ...
            {todayGoals && (
              <>
                ...
                <GoalItem
                  ...
                  current={
                    todayGoals.progress?.questions_practiced?.completed || 0
                  }
                  target={
                    todayGoals.progress?.questions_practiced?.target || 50
                  }
...
function TaskItem({
  ...
  return (
    ...
    >
      ...
      <Badge
        variant="outline"
        className={cn("text-xs", config.color.replace("bg-", "border-"))}
      >
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Top performers this {period === "weekly" ? "week" : "month"}
          </CardDescription>
        </div>
            ...
            {leaderboard.entries.map((entry, idx) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {entry.problems_solved} problems • {entry.streak_days}d
                    streak
                  </p>
                </div>
