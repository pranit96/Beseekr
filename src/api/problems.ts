...
function clearProblemsCache(): void {
  try {
    const keys = Object.keys(localStorage).filter((k) =>
      k.startsWith(CACHE_PREFIX + "problems"),
    );
...
export async function getProblems(
  ...
  try {
    const response = await request<any>(`/api/problems?${params.toString()}`);
    ...
    const result = {
      items: problems.map((problem: any) => ({
        ...
        metrics: {
          ...
          source_count:
            problem.source_count || problem.related_posts?.length || 0,
        },
...
export async function getProblemDetails(id: string): Promise<Problem> {
  ...
  return {
    ...
    competitors:
      response.competitors?.map((comp: any) => ({
        name: comp.name,
    ...
    market_estimate: response.market_size
      ? {
          ...
          confidence: response.feasibility_score
            ? response.feasibility_score / 100
            : undefined,
        }
      : response.market_estimate
        ? {
            size: formatMarketSize(
              response.market_estimate.tam_low,
              response.market_estimate.tam_high,
            ),
    ...
    sources:
      response.related_posts?.map((post: any, idx: number) => ({
...
export async function getPremiumProblems(
  ...
  const response = await request<any>(`/api/problem?${params.toString()}`);
...
async function getResearchStatus(
  ...
  return request<{
    job: { status: string; report_id?: string; error?: string };
