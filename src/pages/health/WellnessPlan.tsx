...
export default function WellnessPlan() {
  ...
  const macros = [
    {
      label: "Calories",
      value: nutrition?.calorie_target,
      unit: "kcal",
      color: "bg-orange-400",
    },
    {
      label: "Protein",
      value: nutrition?.protein_target_g,
      unit: "g",
      color: "bg-sky-400",
    },
    {
      label: "Carbs",
      value: nutrition?.carb_target_g,
      unit: "g",
      color: "bg-emerald-400",
    },
    {
      label: "Fat",
      value: nutrition?.fat_target_g,
      unit: "g",
      color: "bg-violet-400",
    },
  ];
  ...
  const handleRegenerate = async () => {
    try {
      ...
      toast({
        title: "Plan updated",
        description: "Your personalised plan has been regenerated.",
      });
    } catch (err: any) {
      toast({
        ...
        {isLoading && !plan ? (
          ...
          <div className="rounded-2xl border border-border bg-muted/30 p-8 flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Claude is analysing your profile and building a realistic plan.
          ...
          <div className="rounded-2xl border border-border bg-muted/30 p-6 flex flex-col items-center gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-3">
                Complete your health profile, then generate a personalised
              ...
              <div className="flex gap-2 justify-center">
                ...
                <button
                  onClick={handleRegenerate}
                  className="px-3 py-1.5 rounded-lg border border-border text-xs font-medium hover:bg-muted/60"
                >
                  Generate plan
                </button>
              </div>
          ...
          <>
            ...
            <section className="rounded-2xl border border-border bg-gradient-to-br from-emerald-500/10 to-transparent p-4 sm:p-5 space-y-2">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div>
                  <p className="text-[11px] uppercase text-muted-foreground font-medium tracking-wide">
                    Goal
                  </p>
                  <h2 className="text-sm font-semibold capitalize">
                    {plan.overview?.primary_goal?.replace(/_/g, " ") ??
                      "Custom"}{" "}
                    · {plan.overview?.timeframe_weeks ?? 12} weeks
                  </h2>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {plan.overview?.key_focus_areas?.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 rounded-full bg-background text-[11px] border border-border"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <section className="rounded-2xl border border-border bg-muted/30 p-4 sm:p-5 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {macros.map((m) => (
                    <div
                      key={m.label}
                      className="rounded-xl bg-background/60 border border-border/60 p-2.5 text-center"
                    >
                ...
                <p className="text-[11px] text-muted-foreground">
                  Strategy: {nutrition.strategy?.replace(/_/g, " ")}
                </p>
              </section>
            )}
            ...
            {training && (
              <section className="rounded-2xl border border-border bg-muted/30 p-4 sm:p-5 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold">
                    Training · {training.weekly_frequency} sessions/week ·{" "}
                    {training.split?.replace(/_/g, " ")}
                  </h3>
                </div>
                    ...
                    >
                      ...
                      {s.cardio && (
                        <p className="text-[11px] text-muted-foreground mt-1.5">
                          Cardio: {s.cardio.type} · {s.cardio.duration_minutes}{" "}
                          min · {s.cardio.intensity}
                        </p>
                      )}
                    </div>
                  ))}
                    ...
                    >
                      ...
                      {m.approx_macros && (
                        <p className="text-[11px] text-muted-foreground mt-1">
                          ~{m.approx_macros.calories} kcal ·{" "}
                          {m.approx_macros.protein}g P · {m.approx_macros.carbs}
                          g C · {m.approx_macros.fat}g F
                        </p>
                      )}
                    </div>
                  ))}
                ...
                <div className="grid sm:grid-cols-2 gap-2 text-xs">
                  {plan.habits.daily.map((h: any, i: number) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {h.how} · {h.target_per_day} {h.unit}/day
                      </p>
                    </div>
                  ))}
                </div>
