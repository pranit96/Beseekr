...
import { Plus, PiggyBank, DollarSign } from "lucide-react";
  ...
  const handleAddGoalSubmit = async (e: React.FormEvent) => {
    ...
    if (!goalName || !goalTargetAmount || parseFloat(goalTargetAmount) <= 0) {
      toast({
        ...
        description: "Specify a name and positive target amount.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        toast({
          title: "Goal Created",
          description: `"${goalName}" with target ${preferredCurrencySymbol}${parseFloat(goalTargetAmount).toLocaleString()}.`,
        });
  ...
  const handleContributeSubmit = async (e: React.FormEvent) => {
    ...
    if (!contributeGoalId || !contribAmount || parseFloat(contribAmount) <= 0) {
      toast({
        ...
        description: "Specify a positive contribution.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        toast({
          ...
          description: `Contributed ${preferredCurrencySymbol}${parseFloat(contribAmount).toFixed(2)}.`,
        });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Set targets and track progress toward your financial milestones.
          </p>
        </div>
        <Button
      ...
      {goals.length === 0 ? (
        <Card className="p-16 text-center border border-dashed border-border/40 bg-card/50 backdrop-blur-sm flex flex-col items-center justify-center rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground max-w-sm mb-6">
            Define priority targets like an emergency fund or holiday getaway
            and save step by step.
          </p>
          <Button
            onClick={() => setIsAddGoalOpen(true)}
            variant="outline"
            className="gap-1.5 bg-background hover:bg-muted rounded-xl"
          >
                    ...
                    {isContributing && (
                      ...
                      >
                        <Input
                          ...
                          className="h-8 text-sm bg-background rounded-lg font-mono"
                          required
                        />
                        <Input
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            className="flex-1 h-7 text-xs rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-medium"
                          >
          ...
          <form onSubmit={handleAddGoalSubmit} className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="bg-background text-center text-lg rounded-xl"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={goalPriority}
                  onValueChange={(val) => setGoalPriority(val as any)}
                >
