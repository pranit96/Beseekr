...
import {
    ArrowLeft, Brain, Moon, Smile, Frown, Meh, BookOpen, Trash2, Zap, Activity,
} from "lucide-react";
...
export default function WellnessMind() {
    ...
    const { history: moodHistory, logMood, deleteMood, isLoading: moodLoading } = useMood();
    const { history: sleepHistory, logSleep, deleteSleep, isLoading: sleepLoading } = useSleep();
    const { entries: journals, createJournal, deleteJournal, isLoading: journalLoading } = useJournal();
    ...
    // ─── Tab state ──────────────────────────────────────────────────────
    const [activeTab, setActiveTab] = useState<"mood" | "sleep" | "journal">("mood");
    ...
    const handleMoodSubmit = async () => {
        try {
            await logMood.mutateAsync({ moodScore, energyLevel, stressLevel, notes: moodNotes || undefined });
            setMoodNotes("");
            toast({ title: "Mood logged ✓", description: `Score: ${moodScore}/10 ${MOOD_EMOJIS[moodScore - 1]}` });
        } catch (err: any) {
            toast({ title: "Error logging mood", description: err.message, variant: "destructive" });
        }
    };
    ...
    const handleSleepSubmit = async () => {
        ...
        try {
            await logSleep.mutateAsync({
                bedtime, wakeTime, durationHours: duration, quality: sleepQuality,
                notes: sleepNotes || undefined,
            });
            setSleepNotes("");
            toast({ title: "Sleep logged ✓", description: `${duration} hours, quality ${sleepQuality}/5` });
        } catch (err: any) {
            toast({ title: "Error logging sleep", description: err.message, variant: "destructive" });
        }
    };
            ...
            <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur-md">
                <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <p className="text-xs font-medium text-violet-400 uppercase">Mind</p>
                            <h1 className="text-base sm:text-lg font-semibold">Mood · Sleep · Journal</h1>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-4 sm:py-6 space-y-5">
                    ...
                    <div className="space-y-5">
                        ...
                        <section className="rounded-2xl border border-border bg-gradient-to-br from-violet-500/10 to-transparent p-5 space-y-4">
                            <h2 className="text-sm font-semibold flex items-center gap-2">
                                <Smile className="h-4 w-4 text-violet-400" /> How are you feeling?
                            </h2>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-lg font-bold text-violet-400">{moodScore}/10</span>
                                </div>
                                <input
                                    ...
                                    style={{ background: `linear-gradient(to right, #8b5cf6 0%, #8b5cf6 ${(moodScore - 1) / 9 * 100}%, hsl(var(--muted)) ${(moodScore - 1) / 9 * 100}%, hsl(var(--muted)) 100%)` }}
                                />
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs text-muted-foreground flex items-center gap-1">
                                        <Zap className="h-3 w-3 text-amber-400" /> Energy: {energyLevel}
                                    </label>
                                    <input
                                        ...
                                        style={{ background: `linear-gradient(to right, #f59e0b 0%, #f59e0b ${(energyLevel - 1) / 9 * 100}%, hsl(var(--muted)) ${(energyLevel - 1) / 9 * 100}%, hsl(var(--muted)) 100%)` }}
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-xs text-muted-foreground flex items-center gap-1">
                                        <Activity className="h-3 w-3 text-rose-400" /> Stress: {stressLevel}
                                    </label>
                                    <input
                                        ...
                                        style={{ background: `linear-gradient(to right, #f43f5e 0%, #f43f5e ${(stressLevel - 1) / 9 * 100}%, hsl(var(--muted)) ${(stressLevel - 1) / 9 * 100}%, hsl(var(--muted)) 100%)` }}
                                    />
                                </div>
                            </div>
                            ...
                            {/* Notes */}
                            <textarea
                                value={moodNotes} onChange={(e) => setMoodNotes(e.target.value)}
                        ...
                        {last7Moods.length > 0 && (
                            <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-3">
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase">7-day mood trend</h3>
                                <div className="flex items-end gap-1.5 h-24">
                                    {last7Moods.map((m) => (
                                        <div key={m.id} className="flex-1 flex flex-col items-center gap-1">
                                            <span className="text-[10px] font-medium">{m.mood_score}</span>
                                            <div className="w-full rounded-t-lg bg-gradient-to-t from-violet-500/80 to-violet-400/40 transition-all" style={{ height: `${(m.mood_score / maxMood) * 100}%` }} />
                                            <span className="text-[9px] text-muted-foreground">{new Date(m.log_date).toLocaleDateString("en", { weekday: "short" })}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                        ...
                        {moodHistory.length > 0 && (
                            <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-2">
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase">Recent moods</h3>
                                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                                    {moodHistory.slice().reverse().slice(0, 10).map((m) => (
                                        <div key={m.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-background/50 border border-border/50">
                                            <div className="flex items-center gap-2">
                                                <span className="text-lg">{MOOD_EMOJIS[m.mood_score - 1]}</span>
                                                <div>
                                                    <p className="text-xs font-medium">{m.mood_score}/10 · E:{m.energy_level ?? "–"} S:{m.stress_level ?? "–"}</p>
                                                    <p className="text-[10px] text-muted-foreground">{m.log_date}{m.notes ? ` · ${m.notes}` : ""}</p>
                                                </div>
                                            </div>
                                            <button onClick={() => deleteMood.mutateAsync(m.id)} className="text-muted-foreground hover:text-destructive transition">
                ...
                {activeTab === "sleep" && (
                    <div className="space-y-5">
                        <section className="rounded-2xl border border-border bg-gradient-to-br from-indigo-500/10 to-transparent p-5 space-y-4">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs text-muted-foreground">
                                    Duration: <span className="font-semibold text-foreground">{calcDuration(bedtime, wakeTime)} hrs</span>
                                </label>
                            </div>
                                ...
                                <div className="flex gap-2">
                                    {[1, 2, 3, 4, 5].map((q) => (
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-lg text-xs font-medium border transition-all ${sleepQuality === q
                        ...
                        {last7Sleep.length > 0 && (
                            <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-3">
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase">7-day sleep</h3>
                                <div className="flex items-end gap-1.5 h-24">
                                    {last7Sleep.map((s) => {
                                        ...
                                        return (
                                            <div key={s.id} className="flex-1 flex flex-col items-center gap-1">
                                                ...
                                                <div className="w-full rounded-t-lg bg-gradient-to-t from-indigo-500/80 to-indigo-400/40 transition-all" style={{ height: `${Math.min(100, (hrs / maxSleepHrs) * 100)}%` }} />
                                                <span className="text-[9px] text-muted-foreground">{new Date(s.log_date).toLocaleDateString("en", { weekday: "short" })}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        {sleepHistory.length > 0 && (
                            <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-2">
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase">Recent sleep</h3>
                                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                                    {sleepHistory.slice().reverse().slice(0, 10).map((s) => (
                                        <div key={s.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-background/50 border border-border/50">
                                            <div>
                                                <p className="text-xs font-medium">{Number(s.duration_hours) || "–"}h · {"⭐".repeat(s.quality ?? 0)}</p>
                                                <p className="text-[10px] text-muted-foreground">{s.log_date} · {s.bedtime ?? "–"} → {s.wake_time ?? "–"}</p>
                                            </div>
                                            <button onClick={() => deleteSleep.mutateAsync(s.id)} className="text-muted-foreground hover:text-destructive transition">
                                                <Trash2 className="h-3.5 w-3.5" />
                                            </button>
                                        </div>
                                    ))}
                ...
                {activeTab === "journal" && (
                    <div className="space-y-5">
                        ...
                        {journals.length > 0 && (
                            <section className="rounded-2xl border border-border bg-muted/20 p-4 space-y-2">
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase">Past entries</h3>
                                <div className="space-y-2 max-h-96 overflow-y-auto">
                                    {journals.map((j) => (
                                        <div key={j.id} className="px-3 py-2.5 rounded-xl bg-background/50 border border-border/50 space-y-1">
                                            <div className="flex items-start justify-between gap-2">
                                                <p className="text-[10px] text-muted-foreground">{j.entry_date}</p>
                                                <button onClick={() => deleteJournal.mutateAsync(j.id)} className="text-muted-foreground hover:text-destructive transition shrink-0">
                                                    <Trash2 className="h-3.5 w-3.5" />
                                                </button>
                                            </div>
                                            <p className="text-xs text-foreground/90 whitespace-pre-wrap">{j.content}</p>
                                        </div>
                                    ))}
                                </div>
                            </section>
