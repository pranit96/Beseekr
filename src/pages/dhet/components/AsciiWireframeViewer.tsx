...
function renderBlueprintLine(line: string, lineIdx: number) {
  ...
  return (
    <div
      key={lineIdx}
      className="leading-[1.25] tracking-tight hover:bg-violet-500/[0.03] transition-colors"
    >
      {parts}
    </div>
  );
...
export const AsciiWireframeViewer: React.FC<AsciiWireframeViewerProps> = ({
  ...
  const handleDownloadTxt = () => {
    const blob = new Blob([wireframeText], {
  ...
  const handleZoom = (delta: number) =>
    setZoom((p) => Math.min(140, Math.max(70, p + delta)));
  ...
  const CurrentAspectIcon =
    ASPECT_ICONS[aspect as keyof typeof ASPECT_ICONS] || Smartphone;
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {ASPECT_LABELS[aspect as keyof typeof ASPECT_LABELS]}
            </span>
          </button>
          ...
          >
            ...
            <span
              className="text-[11px] font-mono px-1 select-none"
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 0 0 1px rgba(139,92,246,0.06), inset 0 0 80px rgba(0,0,0,0.5)",
        }}
      >
        {/* TWIN: Side-by-Side */}
        {displayMode === "twin" && (
        ...
        {/* DESKTOP FRAME (16:9) */}
        {(displayMode === "blueprint" || displayMode === "mockup") &&
          aspect === "16:9" && (
            <div
